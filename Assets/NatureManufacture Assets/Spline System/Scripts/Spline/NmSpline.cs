...
namespace NatureManufacture.RAM
{
    public class NmSpline : MonoBehaviour
    {
        ...
        [SerializeField] public UnityEvent NmSplineChanged { get; set; }
        ...
        private Keyframe[] GetPointMeshCurveKeyFrames(int i)
        {
            //Debug.Log($"GetPointMeshCurveKeyFrames {i}");
            Keyframe[] keyframes;
            if (MainControlPoints.Count > 0 && i >= 0 && i < MainControlPoints.Count)
            {
                keyframes = MainControlPoints[i].meshCurve.keys;
            }
            else
        ...
        public void AddPoint(Vector4 position)
        {
            ...
            MainControlPoints.Add(new RamControlPoint(position, Quaternion.identity, SnapToTerrain, new AnimationCurve(GetPointMeshCurveKeyFrames(MainControlPoints.Count - 1))));
        }
        ...
        public void AddPoint(Vector4 position, bool pointSnapToTerrain, float pointWidth = 0, AnimationCurve meshCurve = null)
        {
            ...
            MainControlPoints.Add(new RamControlPoint(position, Quaternion.identity, snap, meshCurve ?? new AnimationCurve(GetPointMeshCurveKeyFrames(MainControlPoints.Count - 1))));
        }
        ...
        public void AddPointAtEnd(float pointWidth)
        {
            ...
            MainControlPoints.Add(new RamControlPoint(position, Quaternion.identity, snap, new AnimationCurve(GetPointMeshCurveKeyFrames(MainControlPoints.Count - 1))));
        }
        ...
        public void AddPointAfter(int i, bool pointSnapToTerrain)
        {
            ...
            MainControlPoints.Insert(i + 1, new RamControlPoint(position, rotation, snap, new AnimationCurve(GetPointMeshCurveKeyFrames(i))));
        }
            ...
            float t)
        {
            ...
            float widthPoint = Mathf.Lerp(controlPointsPart[pos].position.w, controlPointsPart[ClampListPos(pos + 1)].position.w, t);
            float snap;

            if (controlPointsPart.Count > pos + 1)
                snap = Mathf.Lerp(controlPointsPart[pos].snap, controlPointsPart[ClampListPos(pos + 1)].snap, t);
            else
                snap = 0;
            ...
            Quaternion rotation = Quaternion.Lerp(controlPointsPart[pos].rotation, controlPointsPart[ClampListPos(pos + 1)].rotation, t);
