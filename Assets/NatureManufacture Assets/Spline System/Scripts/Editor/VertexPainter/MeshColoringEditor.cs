...
namespace NatureManufacture.RAM.Editor
{
    ...
    public class MeshColoringEditor : Editor
    {
            ...
            foreach (var meshFilter in meshFilters)
            {
                ...
                if (meshFilter.sharedMesh != null)
                {
                    ...
                    if (_meshFilterInScene != null)
                        foreach (var meshInScene in _meshFilterInScene)
                        {
                    ...
                    foreach (var item in _ramSplines)
                    {
                        if (item.meshfilter != null)
                            ColorMeshVertex(coloringMesh, thresholdDistance, materialPropertyBlock, item.gameObject, item.meshfilter, ref colors, ref uv3, vertLength, ray, vertices, upVector, ref maxUv);
                        else
                            Debug.Log($"RamSpline {item.name} needs to be regenerated");
                    }

                    foreach (var item in _lakePolygons)
                    {
                        if (item.meshfilter != null)
                            ColorMeshVertex(coloringMesh, thresholdDistance, materialPropertyBlock, item.gameObject, item.meshfilter, ref colors, ref uv3, vertLength, ray, vertices, upVector, ref maxUv);
                        else
        ...
        private static void ColorMeshVertex(MeshColoring coloringMesh, float thresholdDistance, MaterialPropertyBlock materialPropertyBlock, GameObject splineGameObject, MeshFilter splineMeshFilter,
...

        {
            ...
            if (splineMaterial.HasProperty(MeshColoring.DynamicFlowID))
            ...
            for (int i = 0; i < vertLength; i++)
            {
                ...
                bool hited = meshCollider.Raycast(ray, out var hit, 1000);
