...
namespace NatureManufacture.RAM.Editor
{
    public class VertexPainterEditor<T> where T : Component
    {
        ...
        public bool OnSceneGUI(SceneView sceneView)
        {
            ...
            DrawOnMesh(VertexPainterData.ToolbarInt != 0);
            return true;
        }
        ...
        private void DrawOnMesh(bool flowmap)
        {
        ...
        private void DrawFlowMap(Mesh mesh, MeshFilter meshFilter, int vertLength, Vector3[] vertices, Vector3 hitPosition)
        {
            ...
            if (colorsFlowMap.Count == 0)
            {
                for (int i = 0; i < vertLength; i++)
                {
                    colorsFlowMap.Add(new Vector2(0, 0));
                }
            }
            ...
            for (int i = 0; i < vertLength; i++)
            {
                ...
                if (Event.current.shift)
                {
                    colorsFlowMap[i] = Vector2.Lerp(colorsFlowMap[i], new Vector2(0, 0), VertexPainterData.Opacity);
                }
                else
                {
                    ...
                    if (VertexPainterData.FlowToolSelected == 0)
                    {
                        colorsFlowMap[i] = Vector2.Lerp(colorsFlowMap[i], new Vector2(VertexPainterData.FlowDirection, VertexPainterData.FlowSpeed), VertexPainterData.Opacity * distBlend);
                    }
                    else if (VertexPainterData.FlowToolSelected == 1)
                    {
                        ...
                        colorsFlowMap[i] = Vector2.Lerp(colorsFlowMap[i], -direction, VertexPainterData.Opacity * distBlend);
                    }
                    ...
                    else if (VertexPainterData.FlowToolSelected >= 3)
                    {
                        if (_hitPositionOldFlow.magnitude != 0 && Vector3.Distance(_hitPositionOldFlow, hitPosition) > 0.001f)
                        {
                            ...
                            if (VertexPainterData.FlowToolSelected == 3)
                                colorsFlowMap[i] = Vector2.Lerp(colorsFlowMap[i],
                                    -smudgeDirection * VertexPainterData.SpeedMultiplier, VertexPainterData.Opacity * distBlend);
                            else if (VertexPainterData.FlowToolSelected == 4)
                            {
                                colorsFlowMap[i] = colorsFlowMap[i] - smudgeDirection * VertexPainterData.SpeedMultiplier * VertexPainterData.Opacity * distBlend;
                                colorsFlowMap[i] = new Vector2(Mathf.Clamp01(colorsFlowMap[i].x), Mathf.Clamp01(colorsFlowMap[i].y));
                            }
                        }
                    }
                }
