...
using AwesomeTechnologies.VegetationSystem.Biomes;
    ...
    public class RamSpline : MonoBehaviour, IVertexPaintable, ITerrainPainterGetData
    {
        #region Profiles
            ...
            string name = "RamSpline", bool snapToTerrain = false)
        {
            ...
            spline.TerrainManager.NmSpline = spline.nmSpline;
#if UNITY_EDITOR
        ...
        public void GenerateBeginningPointsFromParent()
        {
            ...
            if (beginningSpline.NmSpline.MainControlPoints.Count < 2)
                return;
        ...
        private void GenerateMesh(ref Mesh mesh)
        {
            ...
            for (int i = 0; i < NmSpline.PointsDown.Count; i++)
            {
                ...
                for (int j = 0; j < BaseProfile.vertsInShape * NmSpline.Points[i].Density; j++)
                {
                    ...
                    else if (i == NmSpline.PointsDown.Count - 1 && endingSpline != null && endingSpline.verticesBeginning is {Count: > 0} && endingSpline.normalsBeginning is {Count: > 0})
                    {
                    ...
                    //if (i == 0 || i == NmSpline.PointsDown.Count - 1)
                    //    Debug.Log($"{name} {i} uvs  {uvs4[id].x} {uvs4[id].y} ");
                    offsetVert++;
                }
            }
        ...
        private void GenerateUvs(int j, float pos, IList<Vector3> normals, int id, IList<Vector3> vertices, int i, IList<Vector2> uvs, IList<Vector2> uvs1, IList<Vector2> uvs2, IList<Vector2> uvs3, IList<Vector2> colorsFlowMap)
        {
            float u = 0;
            float u1 = 0;
            float u2 = 0;
        ...
        private void GenerateSplineVertices(Vector3[] vertices, int id, int i, float pos, Vector3[] normals)
        {
            ...
            {
                Terrain checkTerrain = hit.collider.gameObject.GetComponent<Terrain>();
        ...
        private void GenerateEndingVertices(Vector3[] vertices, int id, int j, Vector3[] normals)
        {
        ...
        private void GenerateBeginningVertices(Vector3[] vertices, int id, int j, Vector3[] normals)
        {
